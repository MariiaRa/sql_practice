...
CREATE FUNCTION add_out (IN x int, IN y int, OUT SUM int) AS $$
SELECT x + y;
$$ LANGUAGE SQL;


SELECT *
FROM add_out(666778, 12345678);
...
SELECT *
FROM sum_and_product(11111, 1111);


CREATE TYPE sum_prod AS (SUM int, product int);


CREATE OR REPLACE FUNCTION sum_and_product (int, int) RETURNS sum_prod AS $$
SELECT $1 + $2, $1 * $2;
$$ LANGUAGE SQL;
...
SELECT *
FROM generate_subscripts('{10, -1, 5, 4}'::int[], 1);
